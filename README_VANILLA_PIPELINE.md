# Welcome to the Vanilla Pipeline

This repository contains the infrastructure as code to bootstrap your next CI/CD project. It is developed with security best practices in mind, provides a robust and automated deployment process that utilizes a combination of CDK Stacks primarily written in TypeScript but also deploys Lambda functions which are based in Python. The code is structured in the following way (check the comments, they are important for the future updates):
```bash
├── bin
├── config
├── docs
...
![Deployment Architecture](docs/vanilla-pipeline-architecture.png)
...
In order to use CDK to deploy the vanilla pipeline you need to first install CDK. Instructions how to install can be found [here](https://docs.aws.amazon.com/cdk/latest/guide/getting_started.html)
...
#### Github
To use the vanilla-pipeline for repositories hosted on GitHub you need to establish an AWS CodeStar Connection and use the arn of that connection the next steps.
...
Prepare local environment with the help of the Vanilla Setup tool. Start by running the `./scripts/configure.sh` command and follow the instructions. Check in the Appendix for detailed instructions.
...
4. Commit the modified files and push your changes to the **CORRECT** (not the vanilla-pipeline) repository.

#### Multiple Vanilla Pipeline repositories in same AWS Account(s)
If you want to deploy multiple vanilla pipelines on the same AWS Account you need to make sure they are namespaced differently. If you just clone/fork this repository and deploy the same code in into multiple repositories in the same AWS Account you will have conflicting CloudFormation stacks deployed as they are all namespaced with ```vanillap``` or ```VanillaPipeline```. In order to use different qualifier for the deployment as well as different Application Name please please refer to the section in details: [Change the vanillap qualifier](#change-the-vanillap-qualifier)

## Change the vanillap qualifier
If you want to use another qualifier other than vanillap then you need to use the `./scripts/configure.sh` script to reconfigure the project.
...
Notes:
 - ```CDKToolkit-vanillap``` (this has an exported parameter which collides with the new one in the new CDKToolkit-${newQualifier})
 - Delete the cdk-assets S3 bucket after the stack deletion to not incur any costs

## Keeping Vanilla Pipeline forks up-to-date
Keeping your Vanilla Pipeline forks up to date is a good practice as you benefit from the new features and bug fixes introduced by the new versions. Vanilla Pipeline team actively puts a lot of effort into releasing continuously new versions. We document the new features, common workflows, and changes, all this as part of the [CHANGELOG](./CHANGELOG.md). Migration steps between versions are as well documented and all this is done following the [Semantic Versioning](http://semver.org/). This helps the Vanilla Pipeline consumers to quickly understand the impact of the version upgrades. In any case, updates are optional only controlled by **the responsible pipeline owners**.
...
```bash
VANILLA_PIPELINE_REMOTE=origin; # Ensure the right git remote alias is set, we are assuming "origin" in this case
VERSION_TO_UPDATE_TO=1.1.2;

git merge ${VANILLA_PIPELINE_REMOTE}/${VERSION_TO_UPDATE_TO};
...
* `npm install`                         installs an npm package (make sure to run the npm run npm run validate:fix afterwards)
