...
def get_version(file, name="__version__"):
    ...
    with io.open(path, encoding="utf8") as f:
        exec(f.read(), {}, version_ns) # nosec # nosem - sample code provided by AWS service team
...
def ensure_python(specs):
    ...
    for spec in specs:
        if part == spec:
            return
        try:
            if eval(part + spec):  # nosec # nosem - sample code provided by AWS service team
                return
        except SyntaxError:
            pass
...
def run(cmd, **kwargs):
    ...
    if not isinstance(cmd, (list, tuple)) and os.name != "nt":
        cmd = shlex.split(cmd)
    cmd[0] = which(cmd[0])
    return subprocess.check_call(cmd, **kwargs) #nosec
