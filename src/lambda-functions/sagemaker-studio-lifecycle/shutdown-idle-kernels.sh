...
export SSM_PARAMETER_NAME='/sagemaker/auto-shutdown-script-s3-path'
...
shutdown_script_path=$(aws ssm get-parameter --name ${SSM_PARAMETER_NAME} --query "Parameter.Value" --output text)
aws s3 --endpoint-url https://s3.${AWS_REGION}.amazonaws.com cp ${shutdown_script_path} .auto-shutdown/extension.tar.gz
